...
export const db = {
  ...
  jobs: {
    ...
    findByPoster: async (posterId) => {
      const { data, error } = await supabase.from('jobs').select('*').eq('poster_id', posterId);
      if (error) throw error;
      return data;
    },
    create: async (job) => {
  ...
  applications: {
    ...
    findBySkiller: async (skillerId) => {
      const { data, error } = await supabase.from('applications').select('*').eq('skiller_id', skillerId);
      if (error) throw error;
      return data;
    },
